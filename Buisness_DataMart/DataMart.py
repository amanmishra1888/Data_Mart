...
def add_customer(first_name, last_name, email):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            st.error("No Record found with the given details.")
            conn.close()
            return
        conn.commit()
        conn.close()
        ...
        st.error(f"Data Value Input error: {e}")
...
def add_product(product_name, category, price, stock_quantity):
    try:
        ...
        # Check if any rows were affected
        if cursor.rowcount == 0:
            st.error("Record exist with the given details.")
            conn.close()
            return
        conn.commit()
        conn.close()
...
def delete_order(order_id):
    try:
        ...
        conn.commit()
        conn.close()
        st.success("Deleted Order successfully!")
    except Exception as e:
        # Handle any other errors
        st.error(f"Data error: {e}")
...
def add_order_item(order_id, product_id, quantity, subtotal):
   try:
        ...
        # Check if any rows were affected
        if cursor.rowcount == 0:
            st.error(" Record exist found with the given details.")
            conn.close()
            return
        conn.commit()
        conn.close()
        st.success("Order Item  Created successfully!")
   except Exception as e:
       # Handle any other errors
       st.error(f"Data error: {e}")
...
#To Delete Orders
    ...
    with st.form(key='update_Order_Item_form'):
        ...
        if submit_button:
            add_order_item(order_item_id, product_id, quantity, subtotal)
