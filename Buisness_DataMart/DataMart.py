...
def display_order_items():
    ...
    query = """
            SELECT oi.order_item_id, oi.order_id, p.product_name, oi.quantity, oi.subtotal
            FROM Order_Items oi
...
#To displaya Pie Chart for Order by Date Range
def display_Orders_Date():

    #Query to Get  Orders from a specif Date Range
    query ="SELECT COUNT(order_id) as Order_Count , DATE(order_date) as order_date  FROM orders WHERE order_date BETWEEN '2025-01-01' AND '2025-12-31' GROUP BY order_date ORDER BY Order_Count DESC"

    conn=get_db_connection()
    #storing the values in Datafrae
    Order_date_wise = pd.read_sql(query, conn)
    conn.close()
    #putiing Order_Count and Order_date as Graph Parameters using plotly.express as px

    fig=px.pie(Order_date_wise,values=Order_date_wise['Order_Count'],names=Order_date_wise['order_date'],hover_name='order_date')
    #pie_col.write(fig)
...
def Customer_Spending_Category():
    ...
    query = "SELECT c.first_name as first_name, o.total_amount AS total_spent,p.product_name as product_name FROM orders o JOIN Customers c ON o.customer_id = c.customer_id  INNER JOIN  orders o1 ON o1.customer_id = c.customer_id  INNER JOIN order_items oi ON o1.order_id=oi.order_id INNER JOIN products p  ON oi.product_id =p.product_id"
    #Database Query and Connection
    conn = get_db_connection()
    Customer_Spending_Category = pd.read_sql(query, conn)
    conn.close()
